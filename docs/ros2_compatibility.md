...
| Rolling (Lyrical era) | expected, unvalidated | the vendored snapshot is pinned to rolling and compiles and unit-tests in CI, but a deployed library must be built against current rolling headers; an outdated rolling tree fails as a Jazzy-era contradiction (`docs/internals/rmw.md` lines 262 to 263); the rclcpp bridge is refused post-Jazzy (`era.rs` lines 529 to 535); never run against a rolling node |
...
| Lyrical (and Rolling, identical layout) | expected, unvalidated | the Lyrical and Rolling era arm exists (120-byte `MessageMember` with `is_rosidl_buffer_`, 160-byte init options), and the vendored bindings are Lyrical-era rolling and compile and unit-test in CI, but that path refuses deployment; no build against real Lyrical headers and no run; the rclcpp C++ bridge refuses Lyrical and Rolling at registration by design (`era.rs` lines 524 to 535) | expected, unvalidated | inside the wire rung's stated domain; no run |
