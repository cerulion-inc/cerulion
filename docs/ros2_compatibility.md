...
| Iron | expected, unvalidated | 16-byte GID and the 168-byte init options match Jazzy, and the `MessageMember` arm is the Humble and Iron 112-byte one (`era_pins.rs` lines 76 to 85); the rclcpp bridge takes the Humble message mirror shape, and its service mirror carries Iron's trailing `event_members_` under its own capability cfg, so both typesupport paths compile; never built or run (rclpy) would run; never built or run in this tree |
...
- A Foxy build still fails at compile, by design: the gates refuse the post-Foxy surface it lacks. Humble builds and runs its suite in the `humble` lane; Foxy's lane pins its known refusal until the Foxy work lands.
...
| Humble | built and tested in CI | builds against real Humble headers in the `humble` lane and runs the rmw suite there; the rclcpp bridge mirrors the pre-Iron introspection shape; not yet run on a Humble robot | validated, local rungs | Humble peers on an x86-64 Linux desktop: one attach produced seven `.msg` files byte-identical to `/opt/ros/humble` |
| Iron | expected, unvalidated | the 16-byte GID and 168-byte init options match Jazzy and the `MessageMember` arm is the Humble and Iron 112-byte one and the rclcpp bridge takes that shape; never built or run | expected, unvalidated | inside the wire rung's domain only on the patched releases (rclcpp 21.0.3, rclpy 4.1.3 or later, as applicable); a pre-patch Iron .0 falls to the local rung; no run |
