...
| `FrameEncoder::required_len`/`begin`, every `FrameCursor` and `FrameView` method | No heap allocation on the SUCCESS path (`dynamic_zero_alloc_test`). Error arms may allocate (they carry the field/schema name as a `String`). |
