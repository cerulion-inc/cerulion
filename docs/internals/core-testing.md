...
| `listener_sweep_iox2_test.rs` | The two assumptions the live loop's readiness sweep rests on: the listener doorbell is a LEVEL triggered readiness signal (repeated polls keep saying ready until a drain takes the events), and the sweep reads file descriptors and nothing else (a source walk, since the property is the absence of a read). A third arm walks the pair of idle drains and the pair of unconditional step path drains they lean on, which is the guard for a mutant no behaviour can catch. | parallel | none |
| `event_id_ceiling_iox2_test.rs` | Every event service is created with an event-id ceiling sized to the ids the transport mints, not iceoryx2's 255 default (a listener walks one bitset entry per id on every wait); a source walk catches a creation site that omits it. | parallel | none |
