...
Library model preflight: `cerulion_viz::skeleton::Skeleton::validate_urdf(xml, config)`
checks supported URDF topology, geometry, and measured-motor binding names. Every
movable joint requires exactly one binding; fixed-only models may omit bindings. It
reads no mesh files and installs nothing into Studio. See the supported subset
