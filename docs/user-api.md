...
| `cerulion telemetry status` / `on` / `off` | Show or change usage telemetry for this machine. `status` prints whether it is on and which setting decided it; `on` and `off` persist the choice in `~/.cerulion/telemetry.json`. `DO_NOT_TRACK=1` and `CERULION_TELEMETRY=0` override the file. Works without signing in. Only a build carrying a telemetry key sends anything: the release artifacts, or any build run with `POSTHOG_API_KEY` set; see `docs/telemetry.md` for exactly what is sent. |
