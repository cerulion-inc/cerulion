...
| `CERULION_RECORD_DISCOVERY_SETTLE_MS` | How long the recorder holds bag creation open for late-appearing topics, in ms. This is a CAP: the hold is released as soon as 500 ms have passed with nothing new discovered, so a graph whose topic set is already complete pays that 500 ms and not the cap. Default **2000**. `0` disables the hold. Widen it when a bag reports a topic as `appeared_after_bag_creation`; the equivalent flag is `cerulion bagd --discovery-settle-ms`. |
